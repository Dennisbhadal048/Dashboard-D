...
tab1, tab2, tab3 = st.tabs(["🗺️ Peta Spasial", "📊 Statistik", "📈 Profil"])
...
with tab1:
    ...
    if viz_mode == "Titik (Scatter)":
        ...
        df_grid = pd.DataFrame({'lon': grid_x.flatten(), 'lat': grid_y.flatten(), 'ch': grid_z.flatten()})
        fig = px.scatter_mapbox(df_grid, lat='lat', lon='lon', color='ch', 
                                color_continuous_scale="Jet", zoom=6, height=600, opacity=0.7)
...
with tab2:
    ...
    with col1:
        fig_hist = px.histogram(df_clean, x=actual_ch, title="📊 Histogram - Distribusi Curah Hujan", nbins=50)
    ...
    with col2:
        fig_violin = px.violin(df_clean, y=actual_ch, title="🎻 Violin Plot - Density Distribusi")
