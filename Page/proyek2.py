...
st.markdown(
    """
    <div style="
        background: linear-gradient(180deg, rgba(245,250,255,0.6) 0%, rgba(255,255,255,0) 100%);
        border: 1px solid #eef3f8; border-radius: 12px; padding: 20px; margin-bottom: 22px;
        box-shadow: 0 6px 18px rgba(16,24,40,0.04);
    ">
        <div style="color: #2b8cc4; font-size: 0.85rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;">
            ● Data Engine Core v2.5 | Advanced Schema & Type Cast Matrix
        </div>
        <h2 style="color: #0b3d91; margin: 8px 0; font-size: 1.6rem; font-weight: 600;">
            UNIVERSAL MET-CLIMATE DATA INGESTION
        </h2>
        <p style="color: #475569; max-width: 820px; font-size: 1.0rem; margin: 0;">
...
st.markdown("<h3 style='color: #0b3d91;'>📂 Step 1: Ingestion Berkas Lokal</h3>", unsafe_allow_html=True)
...
if uploaded_file is not None:
    ...
    if file_name.lower().endswith(('.tif', '.tiff')):
        try:
            # Membaca GeoTIFF ringan menggunakan xarray
            da = xr.open_rasterio(uploaded_file)
            
            st.success(f"Berhasil memuat GeoTIFF: {file_name}")
            st.write("**Metadata GeoTIFF:**")
            st.json({
                "Dimensi": dict(da.sizes),
                "Koordinat": list(da.coords),
                "CRS": da.crs if hasattr(da, 'crs') else "Tidak diketahui"
            })
            
            # Konversi ke DataFrame untuk diolah lebih lanjut jika Taruna butuh format tabular
            if st.button("Konversi GeoTIFF ke CSV (1D Format)"):
                df_tif = da.squeeze().to_dataframe(name="Value").reset_index()
                df_tif = df_tif.dropna()
                st.dataframe(df_tif.head(100))
                
        except Exception as e:
            st.error(f"❌ Gagal membaca GeoTIFF. Pastikan file tidak korup. Error: {e}")
            st.stop()
...
# 2. Proses Komputasi Pembacaan Berkas Mentah (DIPERBAIKI)
    ...
    st.markdown("---")
    st.markdown("<h3 style='color: #0b3d91;'>📋 Step 2: Kondisi Berkas Awal (Sebelum Diolah)</h3>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<h3 style='color: #0b3d91;'>🛠️ Step 3: Modifikasi & Transformasi Skema Jenis Data</h3>", unsafe_allow_html=True)
    ...
    with col_clean1:
        ...
        st.markdown("<span style='color:#2b8cc4; font-weight:bold;'>⚙️ Fitur Konversi Jenis Tipe Data:</span>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<h3 style='color: #0b3d91;'>🧠 Step 4: Rekonstruksi Skala Agregasi Waktu</h3>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<h3 style='color: #0b3d91;'>💾 Step 5: Tinjauan Akhir & Ekspor Penyimpanan</h3>", unsafe_allow_html=True)
